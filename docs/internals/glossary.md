...
| Runtime mode        | The thread's permission policy. This fork offers `auto` and `full-access` only for work that runs on another machine. See [permission modes](../user/permission-modes.md). |
